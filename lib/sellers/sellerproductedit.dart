...
class _SellerProductEditState extends State<SellerProductEdit> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      context.read<ProductEditProvider>().initialize(
            widget.productId,
            widget.productData,
          );
    });
  ...
  void dispose() {
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) context.read<ProductEditProvider>().reset();
    });
    super.dispose();
  }

  // ── Searchable Category Sheet ──────────────────────────────────────────────
  void _openCategorySheet(BuildContext context, ProductEditProvider provider) {
  ...
  Widget build(BuildContext context) {
    ...
    return Consumer<ProductEditProvider>(
              ...
              child: Column(
                ...
                children: [
                    ...
                    child: Container(
                      ...
                      child: provider.hasImages
                          ? ListView.builder(
                              ...
                              itemCount: provider.totalImageCount < 6
                                  ? provider.totalImageCount + 1
                                  : provider.totalImageCount,
                              itemBuilder: (context, index) {
                                if (index < provider.existingImageUrls.length) {
                                ...
                                final newIdx =
                                    index - provider.existingImageUrls.length;
                                if (newIdx < provider.webImages.length) {
                                  return _buildNewImageTile(provider, newIdx);
                                }
                                if (provider.totalImageCount < 6) {
                                  return _buildAddImageTile(provider);
                                }
                  ...
                  _buildDropdown(
                    ...
                    onChanged: (val) {
                      if (val != null) provider.refundableController.text = val;
                    },
                  ),
                  12.getHeightWhiteSpacing,
                  _buildField(
                            ...
                            children: [
                              Row(
                                ...
                                children: [
                                  ...
                                  GestureDetector(
                                    onTap: () => provider.removeVariant(index),
                                    child: Container(
  ...
  Widget _buildExistingImageTile(ProductEditProvider provider, int index) {
    return Stack(
      children: [
        Padding(
          padding: const EdgeInsets.all(4),
          child: ClipRRect(
            ...
            child: CachedNetworkImage(
              imageUrl: provider.existingImageUrls[index],
              width: 155,
              height: 155,
              fit: BoxFit.cover,
  ...
  Widget _buildNewImageTile(ProductEditProvider provider, int newIdx) {
    return Stack(
      children: [
        Padding(
          padding: const EdgeInsets.all(4),
        ...
        Positioned(
          ...
          right: 8,
          child: GestureDetector(
            onTap: () => provider.removeNewImage(newIdx),
            child: Container(
              padding: const EdgeInsets.all(4),
              decoration: const BoxDecoration(
                color: Colors.red,
  ...
  Widget _buildAddImageTile(ProductEditProvider provider) {
